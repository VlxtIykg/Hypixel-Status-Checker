  ...
  if (interaction instanceof Eris.CommandInteraction) {
    switch (interaction.data.name) {
      case "check": {
        ...
        if (
          userInfo.code === 429 ||
          userInfo.error === "Too Many Requests"
        ) {
          let slim;
          let custom;
          ...
          // TODO Run tests after fixing errors, then create loop
      ...
      case "componentTestClose": {
        ...
        switch (status) {
          ...
          case "offline.": {
            ...
            // FIXME: Errors to default when component click
            const uuid = await id();
            await interaction.editParent({
    ...
    switch (command) {
      case "loop": {
				const focusIndex = richContentCreation.getFocused({"interaction": interaction, "focused": true});
